...
const instance = {
    ...
    toolbar_checked_id: "phoenix_utils_windows_fund_toolbar_checked",
    checked_id: "phoenix_utils_windows_fund_checked",
...
instance.reload = function (force) {
    ...
    var rows = _.map(data, (row) => _.extend(row, { "checked": _.findIndex(instance.options.checked, (obj) => row["code"] == obj["code"]) >= 0 }));
    ...
    // 设置已选用户
    $$(instance.checked_id).setValue(_.pluck(instance.options.checked, "name").join(","));
}

// 过滤
instance.filter = function () {
    $$(instance.grid_id).filter((row) => ((row["code"] || "") + "|" + (row["name"] || "")).indexOf($$(instance.filter_id).getValue()) >= 0);
    ...
    body: {
        ...
        rows: [
                ...
                rows: [
                    { id: instance.filter_id, view: "text", placeholder: instance.filter_placeholder, clear: true, on: { onChange: instance.filter } },
                    {
                        ...
                        columns: [
                            ...
                            { id: "code", header: { text: "基金代码", css: { "text-align": "center" } }, sort: "text", css: { "text-align": "center" }, width: 120 },
                            ...
                            onCheck(id, col, value) {
                                ...
                                if (value) {
                                    ...
                                    instance.options.checked =
                                        _.reject(instance.options.checked, (user) => user["code"] == row["code"]);
                                }
            ...
            },
            {
                ...
                cols: [{ id: instance.checked_id, view: "text", label: "已选基金", labelAlign: "right", readonly: true, placeholder: "请选择基金..." },]
            },
            {
                cols: [
                    { width: 8 },
...
/*
    {
        ...
        filter      可选    筛选数据回调函数
        checked     可选    已选基金ID，[{"code":"基金代码","name":"基金名称"}, ...]
...
export function fund(options) {
    ...
    if (_.isEmpty(instance.options.checked)) {
        ...
        console.error(`checked must be a Array, like [{"code":"基金代码","name":"基金名称"}, ...]`)
        return
    } else {
    ...
    if (!instance.options.cache || $$(instance.grid_id).count() < 1) {
        ...
        $$(instance.grid_id).eachRow(
            function (id) {
                var row = this.getItem(id);
                row.checked = _.findIndex(instance.options.checked, (obj) => row["code"] == obj["code"]) >= 0;
